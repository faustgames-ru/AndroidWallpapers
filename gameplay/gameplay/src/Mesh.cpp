...
static std::vector<Mesh*> __meshCache;

Mesh::Mesh(const VertexFormat& vertexFormat) 
    : _meshData(NULL), _vertexFormat(vertexFormat), _vertexCount(0), _vertexBuffer(0), _primitiveType(TRIANGLES), 
...
{
    GLuint vbo = createDeviceObject(vertexFormat, vertexCount, dynamic);
...
void Mesh::setDeviceData(MeshData* data)
{
	_meshData = data;
	fillDeviceData();
}

void Mesh::RestoreDeviceObject()
{
	_vertexBuffer = createDeviceObject(_vertexFormat, _vertexCount, _dynamic);
	/*for (unsigned int i = 0; i < _partCount; ++i)
    {
		_parts[i]->RestoreDeviceObject();
    }*/
	fillDeviceData();
}
...
{
	for (std::vector<Mesh*>::iterator itr = __meshCache.begin(); itr != __meshCache.end(); itr++)
	{
		(*itr)->RestoreDeviceObject();
	}
}
...
GLuint Mesh::createDeviceObject(const VertexFormat& vertexFormat, unsigned int vertexCount, bool dynamic)
{
	GLuint vbo;
...
void Mesh::fillDeviceData()
{
	setVertexData((float*)_meshData->vertexData, 0, _meshData->vertexCount);

    _boundingBox.set(_meshData->boundingBox);
    _boundingSphere.set(_meshData->boundingSphere);

	clearMeshParts();
    // Create mesh parts.
    for (unsigned int i = 0; i < _meshData->parts.size(); ++i)
    {
        MeshPartData* partData = _meshData->parts[i];
        GP_ASSERT(partData);

        MeshPart* part = addPart(partData->primitiveType, partData->indexFormat, partData->indexCount, false);
        if (part == NULL)
        {
            GP_ERROR("Failed to create mesh part (with index %d) for mesh '%s'.", i, _url.data());
            SAFE_DELETE(_meshData);
            return;
        }
...
Mesh::MeshPartData::MeshPartData() :
...
Mesh::MeshData::MeshData(const VertexFormat& vertexFormat)
