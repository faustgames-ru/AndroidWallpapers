...
ParticleEmitter* ParticleEmitter::create(const char* textureFile, BlendMode blendMode, unsigned int particleCountMax, ParticleEmitterConstructorProc constructor)
{
	...
	ParticleEmitter* emitter = ParticleEmitter::create(texture, blendMode, particleCountMax, constructor);
    SAFE_RELEASE(texture);
    return emitter;
}

void ParticleEmitter::GenerateParticlePosition(Vector3* dst)
{
	generateVector(_position, _positionVar, dst, _ellipsoid);
}

ParticleEmitter* ParticleEmitter::create(Texture* texture, BlendMode blendMode, unsigned int particleCountMax, ParticleEmitterConstructorProc constructor)
{
	
	ParticleEmitter* emitter = constructor ? constructor(particleCountMax) : new ParticleEmitter(particleCountMax);
    GP_ASSERT(emitter);
...
ParticleEmitter* ParticleEmitter::create(const char* url, ParticleEmitterConstructorProc constructor)
{
	...
	ParticleEmitter* particle = create((strlen(properties->getNamespace()) > 0) ? properties : properties->getNextNamespace(), constructor);
    SAFE_DELETE(properties);

    return particle;
}

ParticleEmitter* ParticleEmitter::create(Properties* properties, ParticleEmitterConstructorProc constructor)
{
	...
	ParticleEmitter* emitter = ParticleEmitter::create(texturePath.c_str(), blendMode, particleCountMax, constructor);
    if (!emitter)
    {
...
void ParticleEmitter::emitOnce(unsigned int particleCount)
{
    ...
    for (unsigned int i = 0; i < particleCount; i++)
    {
		...
		GenerateParticlePosition(&p->_position);
...
Drawable* ParticleEmitter::clone(NodeCloneContext& context)
{
	...
	ParticleEmitter* emitter = ParticleEmitter::create(_spriteBatch->getSampler()->getTexture(), _spriteBlendMode, _particleCountMax, NULL);//!!
	copyData((ParticleEmitter*)emitter);
	return emitter;
}

void ParticleEmitter::copyData(ParticleEmitter* emitter)
{
