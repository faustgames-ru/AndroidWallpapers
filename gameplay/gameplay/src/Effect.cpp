...
void Effect::RestoreDeviceObjects()
{
	std::map<std::string, Effect*>::const_iterator itr = __effectCache.begin();
	for (; itr != __effectCache.end(); itr++)
	{
		itr->second->RestoreDeviceObject();
	}
	//__effectCache
}

void Effect::RestoreDeviceObject()
{
	const char* vshPath = _vshPath.empty() ? NULL : _vshPath.data();//stdStringToChars(_vshPath);//vshPathInput ? vshPathInput :
	const char* vshSource = _vshSource.empty() ? NULL : _vshSource.data();//stdStringToChars(_vshSource);//vshSourceInput ? vshSourceInput :
	const char* fshPath = _fshPath.empty() ? NULL : _fshPath.data();//stdStringToChars(_fshPath);//fshPathInput ? fshPathInput :
	const char* fshSource = _fshSource.empty() ? NULL : _fshSource.data();//stdStringToChars(_fshSource);//fshSourceInput ? fshSourceInput :
	const char* defines = _defines.empty() ? NULL : _defines.data();//stdStringToChars(_defines);//definesInput ? definesInput :
	_program = Effect::createDeviceObjectFromSource(vshPath, vshSource, fshPath, fshSource, defines);
	updateAttributes();
}
...
Effect* Effect::createFromSource(const char* vshPath, const char* vshSource, const char* fshPath, const char* fshSource, const char* defines)
{
    ...
    int program = createDeviceObjectFromSource(vshPath, vshSource, fshPath, fshSource, defines);

    // Create and return the new Effect.
    Effect* effect = new Effect();
    effect->_program = program;
	effect->_vshPath = charsTostdString(vshPath);
    effect->_vshSource = charsTostdString(vshSource);
    effect->_fshPath = charsTostdString(fshPath);
    effect->_fshSource = charsTostdString(fshSource);
    effect->_defines = charsTostdString(defines);

	effect->updateAttributes();

    return effect;
}

int Effect::createDeviceObjectFromSource(const char* vshPath, const char* vshSource, const char* fshPath, const char* fshSource, const char* defines)
{
...
void Effect::updateAttributes()
{
    ...
    GL_ASSERT( glGetProgramiv(_program, GL_ACTIVE_ATTRIBUTES, &activeAttributes) );
    if (activeAttributes > 0)
    {
        GL_ASSERT( glGetProgramiv(_program, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, &length) );
        if (length > 0)
        {
            ...
            for (int i = 0; i < activeAttributes; ++i)
            {
                ...
                GL_ASSERT( glGetActiveAttrib(_program, i, length, NULL, &attribSize, &attribType, attribName) );
                attribName[length] = '\0';

                // Query the pre-assigned attribute location.
                GL_ASSERT( attribLocation = glGetAttribLocation(_program, attribName) );

                // Assign the vertex attribute mapping for the effect.
                _vertexAttributes[attribName] = attribLocation;
            }
