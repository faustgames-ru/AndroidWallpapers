...
//	char *writeTarget; // Used to hold either a block of allocated data or the externally allocated data
	...
	for (sendListIndex=0; sendListIndex < (unsigned)sendListSize; sendListIndex++)
	{
		...
		else
		{
			...
			stringCompressor->EncodeString((const char *)uniqueID, 256, &outgoingBitStream);
		}
...
//	unsigned int bitLength;
	...
	if (nameIsEncoded)
	{
		if ( stringCompressor->DecodeString((char *)&uniqueIdentifier, 256, &incomingBitStream) == false )
		{
#ifdef _DEBUG
			assert( 0 ); // bitstream was not long enough.  Some kind of internal error
#endif
...
/*
		if(uniqueIdentifier != (int *)RPC_PlayerUpdate &&
			uniqueIdentifier != (int *)RPC_UpdateScoresPingsIPs)
			Log("uniqueIdentifier: %d", uniqueIdentifier); // ponpon use this if you wanna identify RPCs
*/
