...
void CharacterGame::initializeChair()
{
	//gameplay-encoder.exe -g HIKCharacterNode_Boy_Reference animations -g greenchair2 chairanimations boyscene.fbx boyscene.gpb
	Node* node = _scene->findNode("greenchair2");
	Animation* animation = node->getAnimation("chairanimations");
	if (animation)
	{
		animation->createClips("res/common/chair.animation");
